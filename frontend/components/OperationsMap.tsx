...
export default function OperationsMap({branches,dcs,gistda,reports,route,impact,layers,selected,onSelect}:Props) {
  ...
  // SVG avoids a device-pixel-ratio-scaled Canvas backing store while retaining one shared renderer.
  useEffect(() => {
  ...
  useEffect(() => {
    const instance=map.current, group=groups.current?.branches, renderer=vectorRenderer.current;
    ...
    return () => { instance.off("moveend zoomend",renderVisible);if(frame!==undefined)window.cancelAnimationFrame(frame);group.clearLayers(); };
  }, [branches]);
